...
**Update 2026-08-06: the converter is no longer in the chain**, so this entry is now history — 4K120 runs over the native HDMI port on a 7.2-rc6 build with FRL. VRR still does not work, but for a different reason: 7.2 shipped FRL *without* HDMI VRR, deliberately (`vrr_range Min: 0 Max: 0`, `vrr_capable 0`). Checked 2026-08-06: Fangzhi Zuo at AMD posted a 4-patch series on 30 July 2026 that adds VRR over the FRL path specifically, Reviewed-by Harry Wentland. It is not merged (and 7.3 does not exist yet), but the patches are public and applicable by hand. The C9 supports 40–120 Hz, so there is a real gain waiting. See [hardware/kernel/](hardware/kernel/README.md).
