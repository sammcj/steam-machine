...
**Update 2026-08-06: the converter is no longer in the chain**, so this entry is now history — 4K120 runs over the native HDMI port on a 7.2-rc6 build with FRL. VRR still does not work, but for a different reason: 7.2 shipped FRL *without* HDMI VRR, deliberately (`vrr_range Min: 0 Max: 0`, `vrr_capable 0`). The ALLM + HDMI VRR series lines up with **7.3**, and the C9 supports 40–120 Hz, so there is a real gain waiting. See [hardware/kernel/](hardware/kernel/README.md).
